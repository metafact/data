...
C.Properties.VariableNames{'OptPrice'} = 'C' %This is for WTI
% C.Properties.VariableNames{'CPrice'} = 'C' %This is for Brent

E = [table(C.K) C];
E.K = []%l.ll ???
E.Properties.VariableNames{'Var1'} = 'K'
E.rf = E.Rate; %This for WTI
% E.rf = E.libor; %For Brent
F = sortrows(E);
D = F;

UnMat = unique(D.MatCell)
if size(UnMat,1) > 1
   warning('More than ONE UNIQUENNNESSS!!!')
   exit 
end
%BLockchain prorgramming
% 
...
k = 1
Settle = unique(D.Date)
Maturity = unique(D.MatCell)
...
newC(:, k) = optstockbyblk(RateSpec,StockSpec,Settle,Maturity,OptSpec,fineK)
...
%         % FOR R
        ...
        %Ka1R = pdfK(end);
        fKa0R = approxCallPDFs(i-1) %try (i-2) here
...
if p < 0.005
    ...
    %     RND(end + 1,:) = {nyRND_L}
    %
%     [avrg, std2, skjevhet, kurts] = est_moments(nyK_L, nyRND_L,S, T0)
%FOR R
          Ks = nyK_L'
          RNDs = nyRND_L'
          St = S
...
%         % FOR R
    ...
    function [avrg, std2, skjevhet, kurts]= est_moments(nyK_L, nyRND_L,S,T)
        
        clear avrg std M2 std2 M3 M4 M3_1 M4_1
        avrg = trapz( log(nyK_L/S), (log(nyK_L/S).*nyRND_L))
    
%         avrg = (log(nyK_L/S)) * (nyRND_L')
        
        std = ( log(nyK_L/S) - avrg ).^2
%         M2 = std*nyRND_L'
       
        M2 = trapz( log(nyK_L/S), (std.*nyRND_L) ); 
        std2 = M2/T
...
%         skjevhet = M3/(M2^(3/2))
        ...
        M3_1 = ( log(nyK_L/S) - avrg ).^3
%         M3 = M3*nyRND_L'
         M3 = trapz(log(nyK_L/S), (M3_1.*nyRND_L) )
        skjevhet = M3/(M2^(3/2))
        
        %
        M4_1 = ( log(nyK_L/S) - avrg ).^4
%         M4 = M4*nyRND_L'
        M4 = trapz(log(nyK_L/S), (M4_1.*nyRND_L) )
        kurts = M4/(M2^(2))
