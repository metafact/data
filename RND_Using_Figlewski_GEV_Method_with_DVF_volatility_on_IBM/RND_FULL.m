%THIS IS TO LOOK FOR THE NAN VALUES IN THE ARRAY
...
for n = 1: length(Unik)
    ...
    i = 499
    p = (newC(i+1)-newC(i-1))/ (fineK(i+1)-fineK(i-1))
    p = p * exp(D.rf(1)*T0) +1 %Formula for cumulative distribution
    if p >= 0.995
    ...
    if p < 0.005
        ...
        [avrg, std2, skjevhet, kurts] = est_moments(nyK_L, nyRND_L)
        snitt = [snitt avrg]
        avvik = [avvik std2]
        skew = [skew skjevhet]
        kurtos = [kurtos kurts]
...
function [avrg, std2, skjevhet, kurts]= est_moments(nyK_L, nyRND_L)

avrg = trapz( nyK_L, (nyK_L.*nyRND_L) )


std = (nyK_L -avrg).^2
std2 = trapz( nyK_L, (std.*nyRND_L) )

%
M2 = std2
M3 = (nyK_L -avrg).^3
M3 = trapz(nyK_L, (M3.*nyRND_L) )
skjevhet = M3/(M2^(3/2))

%
M4 = (nyK_L -avrg).^4
M4 = trapz(nyK_L, (M4.*nyRND_L) )
kurts = M4/(M2^(2))
